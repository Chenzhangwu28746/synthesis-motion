...
from removed.version5.model import EncoderTCN
from removed.version5.model import AttributeTCN
from removed.version5.model import Generator
from removed.version5.model import Discriminator
from removed.version5.dataloader_fourierTransfrom import MyDataset
import removed.version5.data_utils_fourierTransfrom as data_utils
import pathlib
...
legends = ['loss_I', 'loss_C', 'loss_KL', 'loss_D', 'loss_GD', 'loss_GR', 'loss_GC']
...
for epoch in range(num_epochs):
    for batch_idx, (data_a, data_b) in enumerate(dataloader):
        ...
        x_s = frames_s.to(device=device)
        x_a = frames_a.to(device=device)
        ...
        _, c_fake_id = C(x_f)  # 这里原本是用 C 再识别，现在改为 I
        ...
        # 画损失图
        losses = np.concatenate((losses, np.array(
            [loss_I.item(), loss_C.item(), loss_KL.item(), loss_D.item(), loss_GD.item(), loss_GR.item(), loss_GC.item()]).reshape(1, -1)),axis=0)
        ...
        if not os.path.exists('./models/' + CurrentExperiment):
            os.mkdir('./models/' + CurrentExperiment)
        if not os.path.exists('./outputs/' + CurrentExperiment):
            os.mkdir('./outputs/' + CurrentExperiment)
        ...
        if epoch % 2 == 0 and (batch_idx % 5000 == 0 or batch_idx % 5000 == 1):
            ...
            data_utils.save_bvh_to_file(
                './outputs/'+CurrentExperiment+'/轮次{}-批次{}-{}-ID{}.bvh'.format(epoch, batch_idx, tag, label[0]),
                data[0]
            )

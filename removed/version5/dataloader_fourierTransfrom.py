...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir='./v5/walk_id_compacted'):
        ...
        # 将帧数据转为傅里叶变换的频域数据
        self.data = [(skeleton, FT.FFT_2d(frames), label) for skeleton, frames, label
                     in self.data]

        # 计算统计数据 min, max, mean, std
        self.statistics = data_utils.get_data_frames_statistics(self.data)
        np.savetxt('./v5/walk_id_compacted/_min_max_mean_std_FFT.csv', self.statistics)

        # 将帧数据归一化
        self.data = [
            (skeleton, torch.FloatTensor(data_utils.frames_to_normalized_frames(frames, self.statistics)), label) for
            skeleton, frames, label in self.data]
